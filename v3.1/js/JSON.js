...
Blockly.JSON.blockToCode = function (block) {
  ...
  if (block.disabled) return Blockly.JSON.blockToCode(block.getNextBlock());
...
Blockly.JSON.withNextBlocks = function (block, code) {
  ...
  if (nextCode) return [code].concat(nextCode);
  // Singular value inputs aren't lists
  else if (block.outputConnection && block.outputConnection.type == Blockly.OUTPUT_VALUE) return code;
